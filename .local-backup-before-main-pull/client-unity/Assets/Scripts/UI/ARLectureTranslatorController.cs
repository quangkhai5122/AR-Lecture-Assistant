    ...
    public class ARLectureTranslatorController : MonoBehaviour
    {
        ...
        private void Start()
        {
            SetStatus(useMockClient
                ? "Sẵn sàng: Unity mock mode. Hãy quét mặt phẳng rồi bấm Scan."
        ...
        public async Task ScanAndPlaceAsync()
        {
            ...
            try
            {
                ...
                if (placed == 0)
                {
                    SetStatus("Chưa đặt được label. Hãy lia camera chậm qua bảng/slide để nhận diện plane rồi Scan lại.");
                }
                else
                {
